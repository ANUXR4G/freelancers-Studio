...
function CalendarCard(): JSX.Element {
  ...
  return (
    <div className="border border-gray-200 rounded-lg h-full flex flex-col overflow-hidden">
      ...
      {[0, 1, 2, 3, 4].map((row) => (
        <div key={row} className="flex border-t border-gray-200 flex-1 min-h-[60px]">
          {[0, 1, 2, 3, 4, 5, 6].map((col) => {
            ...
            if (inMonth) {
              if (day === 3) events.push(<EventPill key="reel" label="Reel" icon="🎬" />)
              if (day === 13) events.push(<EventPill key="dm" label="DM" icon="🙌" />)
              if (day === 18) events.push(<EventPill key="col" label="Collab" icon="🤝" primary />)
              if (day === 30) events.push(<EventPill key="ln" label="Launch" icon="🔥" />)
            }

            return (
              <div
                key={col}
                className={`flex-1 relative ${col !== 0 ? "border-l border-gray-200" : ""}`}
              >
                <div className="absolute top-1.5 left-2 text-[11px] text-gray-400 font-semibold">
  ...
  return (
    <div className="flex items-end gap-3 h-full pb-4">
      {steps.map((s, idx) => (
        <motion.div
          key={s.n}
          className="relative flex flex-col items-center justify-end rounded-lg border border-gray-200 bg-white text-center shadow-sm flex-1 transition-all"
          style={{ height: s.h }}
        ...
        >
          ...
          <div className="mb-2 mt-auto text-2xl">
            {s.i}
          </div>
          <div className="pb-2 font-semibold text-[15px]">
            {s.t}
          </div>
        </motion.div>
      ))}
...
function MagneticTagsCard(): JSX.Element {
  ...
  return (
    <div
      ref={ref}
      className="min-h-[200px] relative cursor-pointer overflow-hidden"
    ...
    >
      {TAGS.map((t, i) => (
        <motion.span
          ...
          className="px-5 py-2.5 bg-[#ef326c] text-white font-bold rounded-lg text-base shadow-xl whitespace-nowrap pointer-events-none select-none"
...
export default function CustomBento(): JSX.Element {
  return (
    <div className="w-full py-3 px-8 lg:px-16" suppressHydrationWarning>
      {/* Row 1 */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* Calendar Card */}
        <div className="lg:col-span-5">
          <div className="bg-white rounded-2xl shadow-sm p-4 flex flex-col h-full min-h-[300px] lg:min-h-[420px]">
            <div className="mb-3">
              <h5 className="font-bold text-lg mb-1">Plan your growth</h5>
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-2xl shadow-sm p-4 flex flex-col h-full min-h-[300px] lg:min-h-[420px]">
            <div className="mb-3">
              <h5 className="font-bold text-lg mb-1">Our 4-step plan</h5>
              <div className="text-gray-500 text-sm">
                A simple path to smart, scalable growth.
              </div>
            </div>
            <div className="flex-1 min-h-0">
              <StepsCardInner />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-4">
        {/* Percentage Card */}
        <div className="lg:col-span-5">
          <div className="bg-white rounded-2xl shadow-sm p-8 flex flex-col items-center justify-center h-full min-h-[280px] lg:min-h-[220px]">
            <div className="flex-1 flex items-center justify-center">
              <motion.span
                className="text-[#ef326c] font-extrabold tracking-tight leading-none"
            ...
            <motion.div
              className="text-[15px] font-semibold mt-4 text-gray-500 text-center leading-snug"
        ...
        {/* Tags Card */}
        <div className="lg:col-span-7">
          <div className="bg-white rounded-2xl shadow-sm p-4 overflow-hidden h-full min-h-[280px] lg:min-h-[220px]">
            <MagneticTagsCard />
          </div>
        </div>
      </div>
